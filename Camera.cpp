...
void Camera::DrawQuad(const Quad& quad, const Texture& texture, int x_anim, int y_anim ,unsigned int color) const{
	// カメラ座標に変換
	Quad Temp = quad.Translation(-mCenterPos).Scaling(mScale).Rotation(mTheta);
...
void Camera::Update(){
	if(IsShake){// シェイクフラグがtrueの時
		float delta = getDeltaTime();// デルタタイム

		mPosition = mOldPosition;// 元のポジションに戻す
		mCenterPos = mOldCenterPos;

		mPosition.x += My::RandomF(-mRange.x, mRange.x,5);// カメラシェイク
		mPosition.y += My::RandomF(-mRange.y, mRange.y,5);
		setCenterPos();
